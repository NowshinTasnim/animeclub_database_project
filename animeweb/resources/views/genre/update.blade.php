...
<div class="insert_container fix">
    <div class="insert_head fix">{{ __('UPDATE GENRE TABLE') }}</div>
    <div class="insert_form fix">
        <form action="{{ route('genre.update.submit') }}" method="POST">
            @csrf
            <div class="form-group row">
                <div class="insert_left fix">{{ __('Column-name:')}}
                </div>

                <div class="insert_right1 fix">{{ __('Genre Name') }}
                </div>
                ...
                <div class="insert_left fix">{{ __('Where : Genre id')}}
                </div>
